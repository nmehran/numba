...
# NumPy StringDType C-API slots used by the generated IR/native helper.
_NPYSTRING_LOAD_SLOT = 313
_NPYSTRING_RELEASE_ALLOCATOR_SLOT = 318
...
def _native_stringdtype_helper():
    if _STRING_DTYPE is None:
        return None, 0, 0, 0
    try:
        native = importlib.import_module('numba.np.strings._stringdtype')
        if not native.has_stringdtype_api():
            return None, 0, 0, 0
        library = ctypes.CDLL(native.__file__)
        acquire = library.numba_stringdtype_acquire_allocator
        acquire.argtypes = [ctypes.py_object]
        acquire.restype = ctypes.c_void_p
        acquire_two = library.numba_stringdtype_acquire_two_allocators
        acquire_two.argtypes = [
            ctypes.py_object, ctypes.py_object,
            ctypes.POINTER(ctypes.c_void_p),
        ]
        acquire_two.restype = None
        release_two = library.numba_stringdtype_release_two_allocators
...
if _STRING_DTYPE is not None:
    ...
    llvm.add_symbol('numba_NpyString_load',
                    _API_SLOTS[_NPYSTRING_LOAD_SLOT])
    llvm.add_symbol('numba_NpyString_release_allocator',
                    _API_SLOTS[_NPYSTRING_RELEASE_ALLOCATOR_SLOT])
    if _NATIVE_ACQUIRE_ADDR:
        llvm.add_symbol('numba_stringdtype_acquire_allocator',
                        _NATIVE_ACQUIRE_ADDR)
    if _NATIVE_ACQUIRE_TWO_ADDR:
        llvm.add_symbol('numba_stringdtype_acquire_two_allocators',
                        _NATIVE_ACQUIRE_TWO_ADDR)
    if _NATIVE_RELEASE_TWO_ADDR:
        llvm.add_symbol('numba_stringdtype_release_two_allocators',
...
def stringdtype_type_from_dtype(dtype):
    """Return the Numba storage type for a NumPy StringDType dtype."""
    if not is_stringdtype(dtype):
        raise NumbaValueError(f"Unsupported array dtype: {dtype}")
    if not _NATIVE_ACQUIRE_ADDR \
        ...
        raise NumbaValueError(
            ...
            'numba.np.strings._stringdtype helper; run build_ext --inplace',
        )
    return _stringdtype_packet_type(dtype)
...
def _load_string(builder, allocator, packed, intp, byte_ptr):
    ...
    load = cgutils.get_or_insert_function(
        builder.module, load_type, 'numba_NpyString_load',
    )
    ...
    with builder.if_then(same_size):
        ...
        with builder.if_else(empty) as (empty_path, nonempty_path):
            ...
            with nonempty_path:
                ...
                strncmp = cgutils.get_or_insert_function(
                    builder.module, strncmp_type, 'strncmp')
                text_cmp = builder.call(
                    strncmp, [left_buffer, right_buffer, left_size])
                builder.store(
                    builder.icmp_signed('==', text_cmp,
                                        ir.Constant(int32, 0)),
                    result,
                )
...
def _initial_search_value(mode, intp):
    if mode == 'count':
        return ir.Constant(intp, 0)
    return ir.Constant(intp, -1)
...
def _store_unicode_affix_result(builder, result, value_buffer, start_offset,
                                end_offset, slice_valid, unicode_struct,
                                pattern_length, pattern_size, suffix, intp,
                                int8, int32):
    slice_size = builder.sub(end_offset, start_offset)
    empty_pattern = builder.icmp_unsigned(
        '==', pattern_size, ir.Constant(intp, 0))
    ...
    with builder.if_then(builder.and_(slice_valid,
        ...
        builder.store(
            _stringdtype_unicode_region_equal(
                ...
                ir.Constant(intp, 0), pattern_length, intp, int8, int32,
            ),
            result,
        )
...
def _utf8_strncmp_equal(builder, string_size, string_buffer, scalar_data,
    ...
    with builder.if_then(same_size):
        empty = builder.icmp_unsigned('==', string_size, ir.Constant(intp, 0))
        with builder.if_else(empty) as (empty_path, nonempty_path):
            with empty_path:
                builder.store(cgutils.true_bit, result)
            with nonempty_path:
                ...
                strncmp = cgutils.get_or_insert_function(
                    builder.module, strncmp_type, 'strncmp')
                cmp_result = builder.call(strncmp, [string_buffer,
                                                    scalar_data, string_size])
                builder.store(
...
def _byte_nonzero_highbits(builder, word, int64):
    high_bits = ir.Constant(int64, 0x8080808080808080)
    return builder.and_(
        builder.not_(_byte_zero_highbits(builder, word, int64)),
        high_bits,
    )
...
def _unicode_trimmed_length(builder, data, length, kind, intp, int32):
    ...
    decrement = builder.append_basic_block(
        'stringdtype.unicode.trim.decrement')
        ...
        with not_one:
            with builder.if_else(two_byte) as (is_two, not_two):
                with is_two:
                    ...
                    next_pos = builder.add(pos, ir.Constant(intp, 1))
                    _store_utf8_byte(
                        builder, buffer, next_pos, second, int8,
                    )
...
def _stringdtype_unicode_search(builder, value_size, value_buffer, pattern,
    ...
    result = cgutils.alloca_once(builder, intp)
    builder.store(_initial_search_value(mode, intp), result)
    ...
    with builder.if_then(builder.and_(slice_valid,
        ...
        if mode == 'rfind':
            ...
            after = builder.append_basic_block(
                'stringdtype.unicode.rfind.after')
            builder.branch(cond)
            ...
            after = builder.append_basic_block(
                'stringdtype.unicode.find.after')
            builder.branch(cond)
    ...
    def codegen(context, builder, signature, args):
        ...
        if na_name:
            ...
            global_name = cgutils.global_constant(
                builder, '.numba.stringdtype.na_name', name_value,
            )
...
def stringdtype_acquire_allocator(typingctx, array):
    ...
    def codegen(context, builder, signature, args):
        ...
        acquire = cgutils.get_or_insert_function(
            ...
            'numba_stringdtype_acquire_allocator',
        )
        return builder.call(
...
def stringdtype_release_allocator(typingctx, allocator):
    ...
    def codegen(context, builder, signature, args):
        ...
        release = cgutils.get_or_insert_function(
            builder.module, release_type, 'numba_NpyString_release_allocator',
        )
...
def stringdtype_release_allocators(typingctx, allocators):
    ...
    def codegen(context, builder, signature, args):
        ...
        release = cgutils.get_or_insert_function(
            ...
            'numba_stringdtype_release_two_allocators',
        )
...
def _stringdtype_equal_data(typingctx, left_data, left_index, left_allocator,
                            right_data, right_index, right_allocator, invert):
    ...
    def codegen(context, builder, signature, args):
        ...
        result = cgutils.alloca_once(builder, ir.IntType(1))
        builder.store(cgutils.true_bit if invert else cgutils.false_bit,
                      result)
        ...
        with builder.if_then(both_valid):
...
def stringdtype_equal_data(typingctx, left_data, left_index, left_allocator,
    ...
    return _stringdtype_equal_data(
        typingctx, left_data, left_index, left_allocator, right_data,
        right_index, right_allocator, False)


@intrinsic
def stringdtype_not_equal_data(
        typingctx, left_data, left_index, left_allocator, right_data,
        right_index, right_allocator):
    return _stringdtype_equal_data(
        typingctx, left_data, left_index, left_allocator, right_data,
        right_index, right_allocator, True)
...
def _stringdtype_unicode_affix_data(typingctx, value_data, value_index,
    ...
    def codegen(context, builder, signature, args):
        ...
        with builder.if_then(value_valid):
            ...
            _, _, start_offset, end_offset, slice_valid = _normalise_slice(
                builder, value_size, value_buffer, start, end, intp, int8)
            _store_unicode_affix_result(
                builder, result, value_buffer, start_offset, end_offset,
                slice_valid, unicode_struct, pattern_length, pattern_size,
                suffix, intp, int8, int32)
                ...
                with builder.if_then(builder.not_(value_false)):
                    ...
                    with builder.if_then(valid):
                        ...
                        affix_result = cgutils.alloca_once(
                            builder, ir.IntType(1))
                        _store_unicode_affix_result(
                            builder, affix_result, value_buffer, start_offset,
                            end_offset, slice_valid, unicode_struct,
                            pattern_length, pattern_size, suffix, intp, int8,
                            int32)
                        builder.store(
                            _stringdtype_bool(
...
def _stringdtype_byte_search_sliced(
    ...
    with builder.if_then(builder.and_(slice_valid,
        ...
        if mode == 'rfind':
            ...
            builder.store(last, pos)
            cond = builder.append_basic_block('stringdtype.rfind.cond')
            body = builder.append_basic_block('stringdtype.rfind.body')
            decrement = builder.append_basic_block(
                'stringdtype.rfind.decrement')
            after = builder.append_basic_block('stringdtype.rfind.after')
            builder.branch(cond)
            ...
            last = builder.sub(end_offset, pattern_match_size)
            builder.store(start_offset, pos)
            cond = builder.append_basic_block('stringdtype.find.cond')
            body = builder.append_basic_block('stringdtype.find.body')
            advance = builder.append_basic_block('stringdtype.find.advance')
            after = builder.append_basic_block('stringdtype.find.after')
            builder.branch(cond)
...
def _stringdtype_byte_search(builder, value_size, value_buffer,
                             pattern_effective_size, pattern_match_size,
                             pattern_buffer, start, end, mode, intp, int8,
                             int32):
    start_index, end_index, start_offset, end_offset, slice_valid = \
        _normalise_slice(builder, value_size, value_buffer, start, end, intp,
                         int8)
    return _stringdtype_byte_search_sliced(
        builder, value_buffer, start_index, end_index, start_offset,
        end_offset, slice_valid, pattern_effective_size, pattern_match_size,
        pattern_buffer, mode, intp, int8, int32)


def _stringdtype_pattern_match_size(builder, pattern_size,
                                    pattern_effective_size, mode, intp):
    zero = ir.Constant(intp, 0)
    if mode == 'count':
        return builder.select(
            builder.icmp_unsigned('==', pattern_effective_size, zero),
            zero,
            pattern_size,
        )
    one = ir.Constant(intp, 1)
    return builder.select(
        builder.icmp_unsigned('<=', pattern_effective_size, one),
...
@intrinsic
def stringdtype_utf8_search_slice(typingctx, value_data, value_size, start,
                                  end):
    if value_data != types.voidptr \
            or not isinstance(value_size, types.Integer) \
            or not isinstance(start, types.Integer) \
            ...
            builder.store(
                _stringdtype_byte_search_sliced(
                    ...
                    _stringdtype_pattern_match_size(
                        builder, pattern_size, pattern_effective_size, mode,
                        intp),
                    pattern_buffer, mode, intp, int8, int32),
                result,
            )
...
def _stringdtype_utf8_search_data(typingctx, value_data, value_index,
                                  value_allocator, pattern_data, pattern_size,
                                  start, end, mode):
    if value_data != types.voidptr \
            or not isinstance(value_index, types.Integer) \
...
def _utf8_stringdtype_search_sliced_data(
...
def stringdtype_find_utf8_data(typingctx, value_data, value_index,
                               value_allocator, pattern_data, pattern_size,
                               start, end):
    return _stringdtype_utf8_search_data(
        typingctx, value_data, value_index, value_allocator, pattern_data,
...
def utf8_find_stringdtype_sliced_data(
        ...
        end_offset, slice_valid, pattern_data, pattern_index,
        pattern_allocator):
    return _utf8_stringdtype_search_sliced_data(
        typingctx, value_data, start_index, end_index, start_offset,
        end_offset, slice_valid, pattern_data, pattern_index,
