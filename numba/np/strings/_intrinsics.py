...
def register_array_bytes(b, rstrip=True):
    ...
    len_chr = b.size
    size_chr = b.itemsize
    chr_array = frombuffer(b, 'uint8') if b.ndim == 0 \
        else b.view(dtype('uint8'))
    if rstrip:
        return (
...
def register_array_strings(s, rstrip=True):
    ...
    chr_array = frombuffer(s, 'int32') if s.ndim == 0 \
        else s.view(dtype('int32'))
    if rstrip:
...
def _rstrip_inner(chr_array, size_chr, is_scalar=False):
    ...
    if is_scalar or size_chr < 9:
        # Direct iteration for scalars or small arrays
        for i in range(size_stride, chr_array.size, size_chr):
            for p in range(i, i - size_stride - 1, -1):
